...
Spendly is a lightweight personal expense tracker built with Flask and SQLite, developed step-by-step as a teaching project (post-MADD course). Route handlers and the database layer are intentionally left as stubs until their step is reached — do not implement a stub route unless the active task explicitly targets that step.
...
No `tests/` directory exists yet — create one as features gain test coverage.
